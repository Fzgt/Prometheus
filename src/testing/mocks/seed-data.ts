/**
 * =============================================================================
 * 📖 种子数据 (Seed Data / Database Seeding)
 * =============================================================================
 *
 * 用预设的示例数据填充 MSW 内存数据库，让应用启动后就有可展示的内容。
 *
 * 【知识点 - 数据库 Seeding 的概念】
 * Seeding（播种）是数据库开发中的标准术语：
 * 在空数据库中插入初始数据，用于开发、演示或测试。
 *
 * 在真实项目中，seeding 通常通过数据库迁移工具（如 Prisma seed）完成。
 * 这里用 @mswjs/data 的 db.post.create() 模拟这个过程。
 *
 * 【知识点 - 幂等性 (Idempotency)】
 * if (db.post.count() > 0) return;
 * 无论调用多少次 seedDatabase()，结果都一样——只有第一次会插入数据。
 * 这在开发中很重要：
 * - 热更新 (HMR) 会重新执行模块
 * - 如果不检查，每次 HMR 都会插入重复数据
 *
 * 幂等性的定义：f(x) = f(f(x))
 * 在 API 设计中也很重要（如 PUT 应该是幂等的，POST 不是）
 *
 * 【知识点 - 展开运算符复用字段】
 * const author = { authorName: '...', authorAvatar: '...', authorBio: '...' };
 * db.post.create({ ...otherFields, ...author });
 *
 * 所有文章共享同一个作者信息，通过 ...author 展开避免重复。
 * 这是 DRY（Don't Repeat Yourself）原则的简单应用。
 *
 * 【知识点 - Markdown 模板字符串】
 * content 字段使用 ES6 模板字符串（反引号 ` `）编写 Markdown。
 * 模板字符串支持：
 * 1. 多行文本（不需要 \n）
 * 2. 字符串插值（${expression}）
 * 3. 保留缩进和空行
 *
 * 注意：模板字符串中的 \` 需要转义为 \\\`，
 * 这里用 \`\`\` 表示 Markdown 代码块的三个反引号。
 *
 * 【文章内容的教学价值】
 * 这些种子文章不是占位文本（Lorem ipsum），而是真实的技术文章：
 * - React Hooks 深度解析
 * - TypeScript 泛型完全指南
 * - Zustand 生产实践
 * - TanStack Query v5 实战
 * - React Router 7 完全指南
 * - bulletproof-react 架构决策
 * - Vite 构建优化
 * - React 测试最佳实践
 *
 * 这些文章同时作为博客的内容和项目的文档，一举两得。
 * =============================================================================
 */
import { db } from './db';
...
export function seedDatabase() {
	...
	/**
	 * 【共享作者信息】
	 * 所有文章使用同一个作者，通过 ...author 展开到每个 create 调用中。
	 */
	const author = {
...
`,
	...
	/**
	 * 在第一篇文章下创建两条示例评论。
	 * allPosts[0] 是按创建顺序的第一篇文章（React Hooks）。
	 */
